...
**Candidate.** No Windows or Linux builds exist. The `tauri-action` matrix already supports both targets. The blockers are: Python environment packaging for non-macOS and MinerU dependency testing on Linux. See README "Known limitations" item 1 and `docs/HELP-WANTED.md`.
