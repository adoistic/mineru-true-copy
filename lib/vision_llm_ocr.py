...
# Shared semaphore to limit concurrent API requests
_API_SEMAPHORE = threading.Semaphore(8)  # max 8 concurrent requests
    ...
    for model in models:
        for attempt in range(_MAX_RETRIES):
            try:
                ...
                # Gemini models don't support response_format on OpenRouter
        ...
        for img in imgs:
            ...
            for sub_img in img:
                try:
                    ...
                    logger.error(f"[VisionLLM] recognition failed: {exc}")
                    results.append(("[OCR: region unreadable]", 0.0))
